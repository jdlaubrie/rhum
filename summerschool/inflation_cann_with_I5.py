...
if __name__ == '__main__':
    ...
    model_summary = Path(scratch_folder_path, data_path.stem, 'cann_summary.txt')
    ...
    stress_circ_tf = keras.layers.Lambda(function=lambda inputs: stress.stress_circ_th(inputs, stretch2),
                                         name='stress_circ')(
        [dWdI1_biax, dWdI2_biax, dWdI4_biax, dWdI5_biax, stretch1, I1_biax])
    # Stress longitudinal tension
    stress_long_tf = keras.layers.Lambda(function=lambda inputs: stress.stress_long_th(inputs, stretch2),
                                         name='stress_long')(
        [dWdI1_biax, dWdI2_biax, dWdI4_biax, dWdI5_biax, stretch1, I1_biax])
    ...
    force_tf = keras.layers.Lambda(
        lambda x: 1.0e-3 * np.pi * x[3] * (x[1] * (x[2] + x[3]) - 0.5 * x[0] * x[2]) / force_avg,
    ...
    with open(model_summary, 'a') as fh:
        # Pass the file handle in as a lambda function to make it callable
        model_inflation.summary(line_length=80,
    ...
    # Plot the results
    plotter.util.plot_result(input_train, output_train, press_predict,
                             path=Path(scratch_folder_path, 'plot_fitting.pdf'))

    # =======================================================================================#

    predictions = np.zeros([input_train.shape[0], terms])
    lowers = np.zeros([input_train.shape[0], terms])
    uppers = np.zeros([input_train.shape[0], terms])
